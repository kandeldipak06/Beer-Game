...
Game Instructor::createGame()
{
  ...
  Game g;
  g.setGId(gameid);
  int i = this->getInstrId();
  g.setInstrId(i);
  games.push_back(g);
  std::cout << "Instructor created a Game." << std::endl;
  return g;
}
...
vector<Game> Instructor::createGames(int numberOfGames)
{
    ...
    if (numberOfGames == 1)
    {
        ...
        retailerI->move(0, 0);
        wholesalerI->show();
        wholesalerI->move(0, 800);
        distributorI->show();
        distributorI->move(800, 0);
        factoryI->show();
        factoryI->move(800, 800);
    }
...
void Instructor::showGamesStatus()
{
    ...
    {
        if (games[i].getCurrentWeek() < games[i].getWeeksToBePlayed())
        {
            // cout << "Game " << i << " at week " << games[i].getCurrentWeek() << endl;
            // games[i].advanceWeek();
        }
        else
        {
...
void Instructor::setGames(std::vector<Game> gs)
{
    games = gs;
};
