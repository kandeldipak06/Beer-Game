...
{
    const char *sequence = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ123456789!@#$%^&*(){}[]?/.,<>";
    std::string pass;

    // generate password for all players
    std::vector<std::string> password;
    for (int j = 0; j < numberOfPlayers; j++)
    {
        for (int i = 0; i < 8; i++)
        {
            pass[i] = sequence[rand() % 81];
        }
...
void Game::setPlayers(std::vector<Player *> ps)
{
    player_players = ps;
}
...
void Game::setConsumerDemandPerWeek(std::vector<int> cd)
{
    consumerDemandPerWeek = cd;
}

std::vector<int> Game::getConsumerDemandPerWeek()
{
    return consumerDemandPerWeek;
}
void Game::setOrdersToBeExecuted(std::map<int, std::vector<Order>> otbe)
{
    ordersToBeExecuted = otbe;
}
...
void Game::setShipmentsToBeExecuted(std::map<int, std::vector<Shipment>> stbe)
{
    shipmentsToBeExecuted = stbe;
}
...
{
    this->player_players.push_back(player);
};